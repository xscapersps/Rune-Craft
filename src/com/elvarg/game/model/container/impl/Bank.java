...
public class Bank extends ItemContainer {
				...
				for(int bankId = 0; bankId < TOTAL_BANK_TABS; bankId++) {
					if(button == tab_select_start + (bankId * 4)) {
						...
						if(action == 1) {
							...
							for (Item item : items) {
								player.getBank(bankId).switchItem(player.getBank(0), item.clone(), player.getBank(bankId).getSlot(item.getId()), false, false);
							}
	...
	public static void depositItems(Player player, ItemContainer from, boolean ignoreReqs) {
		...
		for (Item item : from.getValidItems()) {
			from.switchItem(player.getBank(Bank.getTabForItem(player, item.getId())), item.clone(), from.getSlot(item.getId()), false, false);
		}
